...
namespace BanhKeo_Doan.FormVaChucNangNghiepVu.FormVaChucNangHoaDonXuatHang
{
    ...
    {
        public ThemHangHoa(string maHoaDonXuat)
        {
            InitializeComponent();
        ...
        {
            // TODO: This line of code loads data into the 'quanLyBanBanhKeo_DoAnDataSet24.Kho' table. You can move, or remove it, as needed.
            this.khoTableAdapter.Fill(this.quanLyBanBanhKeo_DoAnDataSet24.Kho);
            // TODO: This line of code loads data into the 'quanLyBanBanhKeo_DoAnDataSet23.HangHoa' table. You can move, or remove it, as needed.
            //this.hangHoaTableAdapter.Fill(this.quanLyBanBanhKeo_DoAnDataSet23.HangHoa);
            LoadCmbBoxHH();
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtDonGiaChuaCK.Text))
            {
            ...
            string checkQuery = "SELECT COUNT(*) FROM ChiTietHoaDonXuatHang WHERE ChiTietHoaDonXuatHang.MaHangHoa = @MaHangHoa and MaHoaDonXuatHang = @MaHoaDonXuatHang";
            SqlCommand checkCmd = new SqlCommand(checkQuery, conn);
            checkCmd.Parameters.AddWithValue("@MaHangHoa", cmbBoxHangHoa.SelectedValue);
            checkCmd.Parameters.AddWithValue("@MaHoaDonXuatHang", MaHoaDonXuat);
            conn.Open();
            ...
            if (count == 0)
            {
                ...
                string insertQuery = "INSERT INTO ChiTietHoaDonXuatHang (MaHoaDonXuatHang, MaHangHoa, SoLuong, DonGiaTruocCK, PhanTramCK, ChietKhau, DonGiaSauCK, ThanhTien, MaKho, GhiChu) " +
                   "VALUES (@MaHoaDonXuatHang, @MaHangHoa, @SoLuong, @DonGiaTruocCK, @PhanTramCK, @ChietKhau, @DonGiaSauCK, @ThanhTien, @MaKho, @GhiChu)";
                SqlCommand cmd = new SqlCommand(insertQuery, conn);
                cmd.Parameters.AddWithValue("@MaHoaDonXuatHang", MaHoaDonXuat);
                cmd.Parameters.AddWithValue("@MaHangHoa", cmbBoxHangHoa.SelectedValue);
                cmd.Parameters.AddWithValue("@SoLuong", txtSoLuong.Text);
                cmd.Parameters.AddWithValue("@DonGiaTruocCK", txtDonGiaChuaCK.Text);
                cmd.Parameters.AddWithValue("@PhanTramCK", txtPhanTramCK.Text);
                cmd.Parameters.AddWithValue("@ChietKhau", txtSoTienCK.Text);
                cmd.Parameters.AddWithValue("@DonGiaSauCK", txtDonGiaSauCK.Text);
        ...
        private void txtDonGiaChuaCK_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar) && e.KeyChar != '.')
            {
                e.Handled = true;
        ...
        private void cmbBoxHangHoa_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(maHang))
            {
                if (checkBanBuon.Checked)
                {
                    ...
                    txtDonGiaChuaCK.Text = donGia.ToString("N0");
                }
                else
                {
