...
namespace BanhKeo_Doan.FormVaChucNangNghiepVu.FormVaChucNangPhieuXuatRaSX
{
    ...
    {
        private string MaPhieuXuatSX;
       
        public event EventHandler DaThemHangHoaXuatSX;
        public ThemHangHoa(string maphieusanxuat)
        {
            InitializeComponent();
            MaPhieuXuatSX = maphieusanxuat;
        ...
        {
            // TODO: This line of code loads data into the 'quanLyBanBanhKeo_DoAnDataSet53.Kho' table. You can move, or remove it, as needed.
            this.khoTableAdapter.Fill(this.quanLyBanBanhKeo_DoAnDataSet53.Kho);
            // TODO: This line of code loads data into the 'quanLyBanBanhKeo_DoAnDataSet52.HangHoa' table. You can move, or remove it, as needed.
            this.hangHoaTableAdapter.Fill(this.quanLyBanBanhKeo_DoAnDataSet52.HangHoa);

        }

        private void btnChapNhan_Click(object sender, EventArgs e)
        {
            if (cmbKho.SelectedValue == null || cmbTenHang.SelectedValue == null)
            {
            ...
            if (!float.TryParse(txtSoLuong.Text, out float soLuong) || soLuong <= 0 ||
                !float.TryParse(txtGia.Text, out float donGia))
            {
            ...
            using (SqlConnection conn = KetNoiCSDL.GetConnection())
            {
                ...
                kiemTraTrungCmd.Parameters.AddWithValue("@MaPhieuNhap", MaPhieuXuatSX);
                ...
                if (soLuongTrung == 0)
                {
                    ...
                    string maHangHoa = (string)cmbTenHang.SelectedValue;
                    ...
                    string insertQuery = @"
                    INSERT INTO ChiTietPhieuXuatRaSX
                    (MaPhieuXuatSX, MaHangHoa, SoLuong, DonGia, ThanhTien, MaKho, GhiChu)
                    VALUES 
                    (@MaPhieuXuatSX, @MaHangHoa, @SoLuong, @DonGia, @ThanhTien, @MaKho, @GhiChu)";

                    using (SqlCommand cmd = new SqlCommand(insertQuery, conn))
                    {
                        cmd.Parameters.AddWithValue("@MaPhieuXuatSX", MaPhieuXuatSX);
                        ...
                        cmd.ExecuteNonQuery();
                        DaThemHangHoaXuatSX?.Invoke(this, EventArgs.Empty);
        ...
        {
            if (string.IsNullOrWhiteSpace(txtSoLuong.Text) || string.IsNullOrWhiteSpace(txtGia.Text))
            {
                txtThanhTien.Text = "";
                return;
            }
            ...
            if (float.TryParse(txtSoLuong.Text, out float soLuong) && float.TryParse(txtGia.Text, out float donGia))
            {
        ...
        private void cmbTenHang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(maHang))
            {
                ...
                txtGia.Text = donGia.ToString("N0");
