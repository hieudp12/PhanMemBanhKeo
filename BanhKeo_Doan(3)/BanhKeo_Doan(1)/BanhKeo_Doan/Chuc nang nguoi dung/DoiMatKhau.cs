...
namespace BanhKeo_Doan.Chuc_nang_nguoi_dung
{
    public partial class DoiMatKhau : Form
    {
        ...
        private void DoiMatKhau_Load_1(object sender, EventArgs e)
        {
            ...
            KetNoiCSDL ketnoiCSDL = new KetNoiCSDL();
            SqlDataReader reader1 = ketnoiCSDL.ExecuteReader(query);

            if (reader1.Read())
            {
                txtTen.Text = reader1["TenNguoiDung"].ToString();
                txtCv.Text = reader1["ChucVu"].ToString();
                txtTk.Text = reader1["TenDangNhap"].ToString();
            }
        }
    }
}
